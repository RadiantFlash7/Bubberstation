...
/datum/component/tackler/proc/rollTackle(mob/living/carbon/target)
	...
	if(HAS_TRAIT(sacker, TRAIT_TACKLING_TAILED_POUNCE))	//Only given from xenotails
	...
	if(sacker_wing)
		attack_mod += 2
	// BUBBER EDIT START - Taj tackle bonus, same benefit as moths
	var/obj/item/organ/ears/cat/tajaran/sacker_tajaran_ears = sacker.get_organ_slot(ORGAN_SLOT_EARS)	//only tajarans have tajaran ear subtype
